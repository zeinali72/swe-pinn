...
def main(config_path: str):
        ...
        for epoch in range(cfg["training"]["epochs"]):
            ...
            if validation_data_loaded:
                try:
                    ...
                    # Calculate NSE for each component
    ...
    # --- 11. Post-Training (Save & Plot) ---
        ...
        print(">>> Best Model by Combined NSE:")
        print(f"  Epoch:               {best_nse_stats['epoch']+1}")
        print(f"  Combined NSE:        {best_nse_stats['combined_nse']:.6f}")
        print(f"  NSE (h, hu, hv):     {best_nse_stats['nse_h']:.4f}, {best_nse_stats['nse_hu']:.4f}, {best_nse_stats['nse_hv']:.4f}")
        print(f"  RMSE (h):            {best_nse_stats['rmse']:.6f}")
        print(f"  Total Weighted Loss: {best_nse_stats['total_weighted_loss']:.6f}")
        
        print("-" * 50)
        print(">>> Best Model by Total Loss:")
        print(f"  Epoch:               {best_loss_stats['epoch']+1}")
        print(f"  Total Weighted Loss: {best_loss_stats['total_weighted_loss']:.6f}")
        print(f"  Combined NSE:        {best_loss_stats['combined_nse']:.6f}")
        if 'nse_h' in best_loss_stats:
            print(f"  NSE (h, hu, hv):     {best_loss_stats['nse_h']:.4f}, {best_loss_stats['nse_hu']:.4f}, {best_loss_stats['nse_hv']:.4f}")
        if 'rmse' in best_loss_stats:
            print(f"  RMSE (h):            {best_loss_stats['rmse']:.6f}")
        print("="*50 + "\n")
        ...
        if aim_run:
            try:
                ...
                summary_best_nse['nse'] = summary_best_nse['combined_nse'] # Alias for summary
                summary_best_nse['epoch'] = best_nse_stats.get('epoch', 0) + 1
                
                summary_best_loss = best_loss_stats.copy()
                summary_best_loss['nse'] = summary_best_loss.get('combined_nse', -float('inf')) # Alias for summary
        ...
        if ask_for_confirmation():
            if final_params is not None:
                ...
                def plot_gauge(x, y, name, filename):
                    ...
                    gauge_data = None
                    if validation_data_loaded:
                        # Filter validation data for the current gauge location (x, y)
                        # loaded_val_data shape: [N, 6] -> t, x, y, h, u, v
                        dists = jnp.sqrt((loaded_val_data[:, 1] - x)**2 + (loaded_val_data[:, 2] - y)**2)
                        mask = dists < 2.0 # Proximity threshold (meters)
