...
def train_step(
    ...
    def loss_fn(params):
        ...
        Q_target = bc_fn_static(t_inflow) # Returns m^3/s
        flux_target = Q_target / 100.0    # Discharge per unit width (m^2/s)

        U_inflow = model.apply(params, batch['bc_inflow'], train=True)
        h_inflow = U_inflow[..., 0]
        u_inflow = U_inflow[..., 1]
        v_inflow = U_inflow[..., 2]

        # Calculate fluxes: qx = h*u, qy = h*v
        # Inflow is in X direction.
        qx_pred = h_inflow * u_inflow
        
        # Loss: Match target flux in X, zero flux in Y (normal to flow direction)
        loss_inflow_x = jnp.mean((qx_pred - flux_target)**2)
        loss_inflow_y = jnp.mean((h_inflow * v_inflow)**2) # No lateral flow
...
def main(config_path: str):
    ...
    try:
        ...
        aim_run.log_artifact(os.path.abspath(__file__), name='source_script.py')
    ...
    def generate_epoch_data(key):
        ...
        bc_inflow = get_bc_batch(l_in_key, n_bc_per_wall, (0., 0.), (y_inflow_start, domain_cfg["ly"]))
    ...
    # --- 9. Training Loop ---
    best_nse_stats = {'nse': -jnp.inf, 'rmse': jnp.inf, 'epoch': 0, 'global_step': 0, 'total_weighted_loss': 0.0}
    ...
    try:
        for epoch in range(cfg["training"]["epochs"]):
            ...
            avg_total_weighted_loss = float(jnp.sum(batch_total_weighted_loss_stacked)) / num_batches
            ...
            if nse_val > best_nse_stats['nse']:
                best_nse_stats.update({'nse': nse_val, 'rmse': rmse_val, 'epoch': epoch, 'global_step': global_step, 'total_weighted_loss': avg_total_weighted_loss})
                best_params_nse = copy.deepcopy(params)
                if nse_val > -jnp.inf: print(f"    ---> New best NSE: {best_nse_stats['nse']:.6f}")
            
            if avg_total_weighted_loss < best_loss_stats['total_weighted_loss']:
                best_loss_stats.update({'total_weighted_loss': avg_total_weighted_loss, 'epoch': epoch})
            ...
            if aim_run:
                log_metrics(aim_run, step=global_step, epoch=epoch, metrics={
                    ...
                    'training_metrics': {'learning_rate': cfg["training"]["learning_rate"]} # Approximation
                })
    ...
    # --- 10. Post-Training (Save & Plot) ---
        ...
        if ask_for_confirmation():
            if final_params is not None:
                ...
                if aim_run and saved_model_path: aim_run.log_artifact(saved_model_path, name='model_weights.pkl')
                ...
                def plot_gauge(x, y, name, filename):
                    ...
                    plt.savefig(path)
                    plt.close()
                    if aim_run: aim_run.track(Image(path), name=filename)
